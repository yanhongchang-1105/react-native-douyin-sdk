...
public class DouyinModule extends ReactContextBaseJavaModule implements LifecycleEventListener {
    ...
    @ReactMethod
    public void shareLink(final ReadableMap data, Promise promise) {
        this.promise = promise;
        // 分享html
        ContactHtmlObject htmlObject = new ContactHtmlObject();
        // 你的html链接（必填）
        htmlObject.setHtml(data.getString("webpageUrl"));
        // 你的html描述（必填）
        htmlObject.setDiscription(data.getString("description"));
        // 你的html title（必填）
        htmlObject.setTitle(data.getString("title"));
        // 你的html的封面图(远程图片) （选填，若不填，则使用开放平台官网申请时上传的图标）
        htmlObject.setThumbUrl(
                data.getString("thumbImageUrl"));
